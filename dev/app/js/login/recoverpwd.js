...
function myRecover(url) {
  ...
  if (pwd.length < 8) {
    ...
    console.log(par.hash);
    par.pwd = MD5(pwd);
    callWS("GET", "security/recover", par, respRecover);
    return;
  } /*else{
       var response=validar_clave(ps);
       if(response===false){
           var badRecoverR = document.getElementById("badRecoverRestricciones");
            badRecoverR.style.visibility = "visible";
            setTimeout(function() {
                badRecoverR.style.visibility = "hidden";
            }, 5000);
        }else{
