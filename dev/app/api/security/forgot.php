...
// login/forgot
    ...
    $sql =  "UPDATE customers " .
            ...
            "WHERE  UPPER(contactemail)='".strtoupper($email)."' ";
    ...
    $sql = "Select name FROM customers WHERE UPPER(contactemail)='".strtoupper($email)."'";
    if (!$rs=$db->query($sql))
        badEnd("500", array("sql"=>$sql,"msg"=>$db->error));
    ...
    $subject = "Recuperación de clave APP DaycoPrint";    
    $body = 
        "<html>" .
        "<head>" .
        "<title></title>" .
    ...
    "<br/>Recuperaci&oacute;n de Clave de la APP<br/>&nbsp;" .
    "</div>" .
    "</div>" .
    "<div style='display:table-row'>" .
    ...
    "<a href=$homeurl../app/?id=login&sid=recoverpwd&hash=".$hash."&email=".$email.">" .
